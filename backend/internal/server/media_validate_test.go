...
func TestValidateUploadedObjectImage(t *testing.T) {
	...
	got, err := validateServer(sr).validateUploadedObject(context.Background(), "me-id", "users/me-id/uploads/x.png")
	if err != nil {
...
func TestValidateUploadedObjectVideo(t *testing.T) {
	...
	got, err := validateServer(sr).validateUploadedObject(context.Background(), "me-id", "users/me-id/uploads/v.mp4")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestValidateUploadedObjectForeignPrefix(t *testing.T) {
	sr := &fakeStorageRepo{headInfo: &storage.ObjectInfo{ContentType: "image/png", SizeBytes: 1000}}
	_, err := validateServer(sr).validateUploadedObject(context.Background(), "me-id", "users/other-id/uploads/x.png")
...
func TestValidateUploadedObjectTraversal(t *testing.T) {
	...
	for _, key := range []string{
		...
		if _, err := validateServer(sr).validateUploadedObject(context.Background(), "me-id", key); !errors.Is(err, errInvalidStorageKey) {
...
func TestValidateUploadedObjectMissing(t *testing.T) {
	...
	_, err := validateServer(sr).validateUploadedObject(context.Background(), "me-id", "users/me-id/uploads/x.png")
...
func TestValidateUploadedObjectInvalidMime(t *testing.T) {
	...
	_, err := validateServer(sr).validateUploadedObject(context.Background(), "me-id", "users/me-id/uploads/x.gif")
